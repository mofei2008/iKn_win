...
def method(url, method, data=None,headers=None):
    """ 定义一个主函数，根据method是get或post，来调用send_post()或send_get() """

    requests.packages.urllib3.disable_warnings()

    if method == 'post':
        res = requests.post(url=url, data=data,headers=headers,verify=False)  # 如果是POST请求，则调用send_post()
        # r = res.json()
    else:
        res = requests.get(url=url, data=data,headers=headers,verify=False)  # 如果是GET请求，则调用send_get()
        # r = res.json()
    return res  # 将结果返回出去
...
def get_token_akn_android():
    url = 'http://icanapi.aikenong.com.cn/version2.0/login/'
    headers = {'Content-Type': 'application/json;charset=UTF-8'}
    par = {"username": '18610806332', "password": 'Aa111111'}
    resp = requests.post(url, data=json.dumps(par), headers=headers)
    token = resp.json()['data']['token']
    # token = login()
    print(token)
